# Documentation Hub

This hub points to the documentation that is already authoritative for the
repository. It separates frozen scientific evidence from additive software
platform guides and research workflow materials.

## Start here

- [Architecture](architecture.md)
- [Quick Start](quick_start.md)
- [Developer Guide](developer_guide.md)
- [Public API](public_api.md)
- [Repository architecture snapshot](repository_architecture.md)
- [Project health](project_health.md)

## Research use

- [Research Guide](research_guide.md)
- [CLI Guide](cli_guide.md)
- [Validation Guide](validation_guide.md)
- [Reproducibility Guide](reproducibility_guide.md)
- [Publication Guide](publication_guide.md)
- [Session inventory](session_inventory/session_index.md)
- [Traceability graph](traceability/README.md)
- [Scientific literature and endpoint mapping](scientific/paper_inventory.md)

## Navigation

- [Module Index](module_index.md)
- [Glossary](glossary.md)
- [FAQ](faq.md)
- [Roadmap](roadmap.md)
- [V3 migration reports](migration/README.md)
- [Vietnamese documentation](vi/README.md)
- [Final audit package](final_audit/repository_certification.md)
- [Archive package](archive/archive_readme.md)
- [Submission package](submission/supplementary_index.md)

The canonical manuscript is [docs/report/final_report.md](report/final_report.md).
Frozen final artifacts are listed in [the publication package](publication/publication_package.md).
